...
using System.IO;
using Study.DevExpressForm.Properties;
using System.Threading;
using Study.DevExpressForm.Job;

namespace Study.DevExpressForm
{
    public partial class JobSearchForm : DevExpress.XtraEditors.XtraForm
    {
        private DataTable dt1 = null;//表结构
        private DataTable dt2 = null;//表结构
        private DataTable dt3 = null;//表结构
        private SerializableDictionary<string, string> dic = null;
        private SerializableDictionary<string, string> dic2 = null;
        private List<JobInfo> jobInfoList1 = new List<JobInfo>();//搜索到的记录(猎聘网)
        private List<JobInfo> jobInfoList2 = new List<JobInfo>();//搜索到的记录(前程无忧)
        private List<JobInfo> jobInfoList3 = new List<JobInfo>();//搜索到的记录(智联招聘)
        private JobInfo curJobInfo1 = null;//当前记录
        private JobInfo curJobInfo2 = null;//当前记录
        private JobInfo curJobInfo3 = null;//当前记录
        ...
        {
            timer1.Enabled = true;//启动状态的计时控件
            InitStatusBar();

            dt1 = typeof(JobInfo).ToDataTable();
            dt2 = typeof(JobInfo).ToDataTable();
            dt3 = typeof(JobInfo).ToDataTable();
            using (TextReader reader = new StringReader(Resources.city))
            {
                XmlSerializer xmlFormatter = new XmlSerializer(typeof(SerializableDictionary<string, string>));
                this.dic = (SerializableDictionary<string, string>)xmlFormatter.Deserialize(reader);
            }
            using (TextReader reader = new StringReader(Resources.city2))
            {
                XmlSerializer xmlFormatter = new XmlSerializer(typeof(SerializableDictionary<string, string>));
                this.dic2 = (SerializableDictionary<string, string>)xmlFormatter.Deserialize(reader);
            }
        }
        #endregion
        ...
        {
            string workAddress = this.txtAddress.Text.Trim();
            if (string.IsNullOrEmpty(workAddress))
            {
                XtraMessageBox.Show("工作地点不能为空!");
                return;
            }

            //--加载猎聘网的数据
            ThreadPool.QueueUserWorkItem(new WaitCallback(GetLiePinData));
            //--加载前程无忧的数据
            ThreadPool.QueueUserWorkItem(new WaitCallback(Get51JobData));
            //--加载智联招聘的数据
            ThreadPool.QueueUserWorkItem(new WaitCallback(GetZhiLianData));
        }
        #endregion

        #region * 猎聘网
        private void GetLiePinData(object obj)
        {
            string workAddress = this.txtAddress.Text.Trim();
            string workAddressId = string.Empty;
            string keyWord = this.txtKeyWord.Text.Trim();
            string upperSalary = this.txtSalary1.Text.Trim();
            string lowerSalary = this.txtSalary2.Text.Trim();
            string mustKey = string.Empty;
            ...
            KeyValuePair<string, string> kv = dic.FirstOrDefault(t => t.Value.Contains(workAddress));
            if (kv.Key == null)
            {
                XtraMessageBox.Show("无法搜索该工作地点", "警告", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            workAddressId = kv.Key;
            ...
            JobFactory tws = new JobFactory("猎聘", workAddress, workAddressId, keyWord, upperSalary, lowerSalary, mustKey);
            IJob job = tws.GetJob();
            if (job != null)
            {
                job.GetJobEnd -= new GetJobEndEventHandler(job_GetJob1End);
                job.GetJobEnd += new GetJobEndEventHandler(job_GetJob1End);
        ...
        private void job_GetJob1End(object o, JobInfo e)
        {
            this.Invoke((MethodInvoker)delegate
            {
                if (e != null)
        ...
        private void gvJob1_InitNewRow(object sender, DevExpress.XtraGrid.Views.Grid.InitNewRowEventArgs e)
        {
            try
            {
                DataRowView dr = this.gvJob1.GetRow(e.RowHandle) as DataRowView;
                dr["Url"] = curJobInfo1.Url;
                dr["Position"] = curJobInfo1.Position;
                dr["Company"] = curJobInfo1.Company;
                dr["Nature"] = curJobInfo1.Nature;
                dr["Scale"] = curJobInfo1.Scale;
                dr["Salary"] = curJobInfo1.Salary;
                dr["Address"] = curJobInfo1.Address;
                dr["Experience"] = curJobInfo1.Experience;
                dr["Education"] = curJobInfo1.Education;
                dr["Time"] = curJobInfo1.Time;

                this.gvJob1.UpdateCurrentRow();
                this.gvJob1.RefreshData();
                this.gvJob1.MoveLast();
            }
            catch
            {
        ...
        private void gvJob1_DoubleClick(object sender, EventArgs e)
        {
            string uri = this.gvJob1.GetFocusedDataRow()["Url"].ToString();
        ...
        private void Get51JobData(object obj)
        {
            ...
            string workAddressId = string.Empty;//工作地点ID
            ...
            string mustKey = string.Empty;//是否包含关键词
        ...
        private void gvJob2_InitNewRow(object sender, DevExpress.XtraGrid.Views.Grid.InitNewRowEventArgs e)
        {
            try
            {
                DataRowView dr = this.gvJob2.GetRow(e.RowHandle) as DataRowView;
                dr["Url"] = curJobInfo2.Url;//网站链接
                dr["Position"] = curJobInfo2.Position;//职位名称
                dr["Company"] = curJobInfo2.Company;//公司名称
                dr["Nature"] = curJobInfo2.Nature;//公司性质
                dr["Scale"] = curJobInfo2.Scale;//公司规模
                dr["Salary"] = curJobInfo2.Salary;//月薪/年薪
                dr["Address"] = curJobInfo2.Address;//工作地点
                dr["Experience"] = curJobInfo2.Experience;//工作经验
                dr["Education"] = curJobInfo2.Education;//最低学历
                dr["Time"] = curJobInfo2.Time;//发布时间
        ...
        #region * 智联招聘
        private void GetZhiLianData(object obj)
        {
            string workAddress = this.txtAddress.Text.Trim();
            string workAddressId = string.Empty;
            string keyWord = this.txtKeyWord.Text.Trim();
            ...
            JobFactory tws = new JobFactory("智联", workAddress, workAddressId, keyWord, upperSalary, lowerSalary, mustKey);
            IJob job = tws.GetJob();
            if (job != null)
            {
                job.GetJobEnd -= new GetJobEndEventHandler(job_GetJob3End);
                job.GetJobEnd += new GetJobEndEventHandler(job_GetJob3End);
                th = new Thread(new ThreadStart(job.GetJobInfoList));
                th.IsBackground = true;
                th.Start();
        ...
        private void job_GetJob3End(object o, JobInfo e)
        {
            this.Invoke((MethodInvoker)delegate
            {
                if (e != null)
        ...
        private void gvJob3_InitNewRow(object sender, DevExpress.XtraGrid.Views.Grid.InitNewRowEventArgs e)
        {
            try
            {
                DataRowView dr = this.gvJob3.GetRow(e.RowHandle) as DataRowView;
                dr["Url"] = curJobInfo3.Url;
                dr["Position"] = curJobInfo3.Position;
                dr["Company"] = curJobInfo3.Company;
                dr["Nature"] = curJobInfo3.Nature;
                dr["Scale"] = curJobInfo3.Scale;
                dr["Salary"] = curJobInfo3.Salary;
                dr["Address"] = curJobInfo3.Address;
                dr["Experience"] = curJobInfo3.Experience;
                dr["Education"] = curJobInfo3.Education;
                dr["Time"] = curJobInfo3.Time;

                this.gvJob3.UpdateCurrentRow();
                this.gvJob3.RefreshData();
                this.gvJob3.MoveLast();
            }
            catch
            {
                XtraMessageBox.Show("添加行失败");
        ...
        private void gvJob3_DoubleClick(object sender, EventArgs e)
        {
            string uri = this.gvJob3.GetFocusedDataRow()["Url"].ToString();
            if (!string.IsNullOrEmpty(uri))
            {
                System.Diagnostics.Process.Start(uri);
            }
        ...
        {
            this.txtAddress.EditValue = null;
            this.txtKeyWord.EditValue = null;
            this.txtSalary1.EditValue = null;
            this.txtSalary2.EditValue = null;
            this.txtMustKey.EditValue = null;
            this.gcJob1.DataSource = null;
        }
        #endregion

        #region * 初始化状态栏
        DevExpress.XtraBars.BarStaticItem newStatItem1 = new DevExpress.XtraBars.BarStaticItem();

        #region * 属性
        private static string[] _toolBarText = { "", "", "" };
        /// <summary>
        /// 态栏文本内容
        /// </summary>
        public static string[] ToolBarText
        {
            ...
            set { _toolBarText = value; }
        }

        private static string[] _toolBarAlignment = { "left", "left", "left" };
        /// <summary>
        /// 态栏位置
        /// </summary>
        public static string[] ToolBarAlignment
        {
            get { return _toolBarAlignment; }
            set { _toolBarAlignment = value; }
        }

        private static string[] _TextAlignment = { "left", "left", "left" };
        /// <summary>
        /// 状态栏文本位置
        /// </summary>
        ...
        void InitStatusBar()
        {
            try
            {
                StatusBar.ClearLinks();
                DevExpress.XtraBars.BarStaticItem newStatItem = new DevExpress.XtraBars.BarStaticItem();
                newStatItem.Caption = "QQ群：186841119";
                newStatItem.Alignment = DevExpress.XtraBars.BarItemLinkAlignment.Left;
                newStatItem.TextAlignment = StringAlignment.Far;
                StatusBar.AddItem(newStatItem);

                newStatItem = new DevExpress.XtraBars.BarStaticItem();
                newStatItem.Caption = _toolBarText[0];
                switch (_toolBarAlignment[0])
                {
                    case "left":
                        newStatItem.Alignment = DevExpress.XtraBars.BarItemLinkAlignment.Left;
                        break;
                    case "right":
                        newStatItem.Alignment = DevExpress.XtraBars.BarItemLinkAlignment.Right;
                        break;
                    case "default":
                        newStatItem.Alignment = DevExpress.XtraBars.BarItemLinkAlignment.Default;
                        break;
                    default:
                        newStatItem.Alignment = DevExpress.XtraBars.BarItemLinkAlignment.Default;
                        break;
                }
                switch (_TextAlignment[0])
                {
                    case "left":
                        newStatItem.TextAlignment = StringAlignment.Far;
                        break;
                    case "right":
                        newStatItem.TextAlignment = StringAlignment.Near;
                        break;
                    case "center":
                        newStatItem.TextAlignment = StringAlignment.Center;
                        break;
                    default:
                        newStatItem.TextAlignment = StringAlignment.Far;
                        break;
                ...
                newStatItem1 = new DevExpress.XtraBars.BarStaticItem();
                newStatItem1.Caption = "";// "当前时间：" + DateTime.Now.ToString();
                newStatItem1.Alignment = DevExpress.XtraBars.BarItemLinkAlignment.Left;
                newStatItem1.TextAlignment = StringAlignment.Far;
                StatusBar.AddItem(newStatItem1);

                newStatItem = new DevExpress.XtraBars.BarStaticItem();
                newStatItem.Caption = _toolBarText[2];
                switch (_toolBarAlignment[2])
                {
                    case "left":
                        newStatItem.Alignment = DevExpress.XtraBars.BarItemLinkAlignment.Left;
                        break;
                ...
                newStatItem = new DevExpress.XtraBars.BarStaticItem();
                newStatItem.Caption = "联系电话:18922735098";
                newStatItem.Alignment = DevExpress.XtraBars.BarItemLinkAlignment.Left;
                newStatItem.TextAlignment = StringAlignment.Near;
                StatusBar.AddItem(newStatItem);

                newStatItem = new DevExpress.XtraBars.BarStaticItem();
                newStatItem.Caption = "Copyright © JARA 2014-2015";
                newStatItem.Alignment = DevExpress.XtraBars.BarItemLinkAlignment.Right;
                newStatItem.TextAlignment = StringAlignment.Near;
                StatusBar.AddItem(newStatItem);
            }
            ...
            {
                //WriteLogAndShowMessageBox.Error("初始化状态栏失败:" + exMsg.Message, "用户[" + currentuser.s_usercode + "]登录初始化状态栏失败:" + exMsg.ToString());
                throw exMsg;
            }
        }
        #endregion
        ...
        {
            Calendar calender = new Calendar(DateTime.Now);
            string calendar = "当前时间：" + DateTime.Now + " ";
            calendar += "(" + calender.ChineseDateString + " ";
    ...
    {
        internal static DataTable ToDataTable(this Type type)
        {

            var dt = new DataTable();

            type.GetProperties().ToList().ForEach(
                c => dt.Columns.Add(c.Name, Nullable.GetUnderlyingType(c.PropertyType) ?? c.PropertyType));
            return dt;
        }
    }
    ...
    public class Calendar
    {
        ...
        private Int32 _cYear;
        private Int32 _cMonth;
        private Int32 _cDay;
        ...
        public string NextDateHoliday
        {
            get
            {
                ...
                //当月剩下的节日
                List<SolarHolidayStruct> cHoliday = (from info in sHolidayInfo
                                                     where info.Month == month && info.Day > day
                                                     orderby info.Day ascending
                                                     select info
                                                          ).ToList<SolarHolidayStruct>();
                //下个月节日
                if (cHoliday.Count == 0)
                {
                    cHoliday = (from info in sHolidayInfo
                                where info.Month == month + 1
                                orderby info.Day ascending
                                select info
                                ).ToList<SolarHolidayStruct>();
                }
                if (cHoliday.Count > 0)
                {
                    result = cHoliday[0].HolidayName + "|" + cHoliday[0].Month + "月" + cHoliday[0].Day + "日";
                }
                return result;
            }
        }
        ...
        public Calendar(DateTime dt)
        {
            ...
            //计算两天的基本差距
            TimeSpan ts = _date - Calendar.MinDay;
            offset = ts.Days;
        ...
        /// //传回农历y年m月的总天数
        /// </summary>
        private Int32 GetChineseMonthDays(int year, int month)
        {
        ...
        /// 传回农历y年闰月的天数
        /// </summary>
        private Int32 GetChineseLeapMonthDays(int year)
        {
        ...
        /// 取农历年一年的天数
        /// </summary>
        private Int32 GetChineseYearDays(int year)
        {
            int i, f, sumDay, info;
        ...
        /// 将星期几转成数字表示
        /// </summary>
        private Int32 ConvertDayOfWeek(DayOfWeek dayOfWeek)
        {
            switch (dayOfWeek)
            {
