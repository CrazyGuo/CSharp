...
{
    /// <summary>
    /// Information about a data provider.具体实现数据库驱动的类 包括IConnection ICommand等接口  使用了反射的思想
    /// </summary>
	[Serializable]
    [DebuggerDisplay("DbProvider: {Id}-{AssemblyName}")]
		...
		public void Initialize()
		{
			...
			catch(Exception e)
			{
				throw new ConfigurationException(
					string.Format("Could not configure providers. Unable to load provider named \"{0}\" not found, failed. Cause: {1}", id, e.Message), e
					);
			}
		}
...
//			}
		...
		{
			return (id.GetHashCode() ^ _assemblyName.GetHashCode() ^ _connectionClass.GetHashCode());
		}
