...
namespace MyBatis.Common.Xml
{
	...
	public sealed class NodeUtils
	{
		...
		{
			string value = attributes[name];//主要是根据name获得值 
			if (value == null) 
			{
				return string.Empty;//不在则返回一个默认值
			}
		    return value;
		}
		...
		public static string ParsePropertyTokens(string str, NameValueCollection  properties) 
		{
			...
			{
				int start = newString.IndexOf(OPEN);//第一次处理${}类型
				...
				while (start > -1 && end > start) 
				{
					...
					if (propValue == null) 
					{
						newString = prepend + propName + append;//没有值 则私用属性名
					} 
					else 
					{
						newString = prepend + propValue + append;//有值 则使用获得的值
					}
					start = newString.IndexOf(OPEN);//循环处理${}类型
