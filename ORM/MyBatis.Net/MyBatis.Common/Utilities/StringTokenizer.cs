	...
	public sealed class StringTokenizer : IEnumerable 
	{
		...
		private sealed class StringTokenizerEnumerator : IEnumerator 
		{
			...
			private string GetNext() 
			{
				...
				char c = stokenizer.origin[cursor];//获取字符串cursor下的字符
                bool isDelim = (stokenizer.delimiters.IndexOf(c) != -1);//判断当前字符是否在delimiters中

                if (isDelim) //如果在delimiters中
				{
					cursor++;
